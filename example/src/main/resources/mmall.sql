...
CREATE TABLE `mmall_order` (
  ...
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `order_no_index`  USING BTREE(`order_no`)
...
CREATE TABLE `mmall_order_item` (
  ...
  PRIMARY KEY (`id`),
  KEY `order_no_index`  USING BTREE(`order_no`),
  KEY `order_no_user_id_index`  USING BTREE(`user_id`,`order_no`)
...
CREATE TABLE `mmall_user` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `user_name_unique`  USING BTREE(`username`)
